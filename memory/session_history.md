...
**问题排查记录**:
1. **依赖安装**: 首次启动需安装 flask, flask-cors, schedule, akshare, pandas-ta 等依赖
2. **导入路径**: Web应用需使用 `from src.xxx` 而非 `from xxx`（因通过 `python -m src.main` 启动）
3. **网络代理**: 系统代理导致 akshare 数据获取失败，需配置跳过代理或修复代理设置
4. **中文编码**: 历史数据获取时 `"日k"` 参数出现编码问题，需修复 akshare 源码中的字符串处理
