...
1. **发现阶段**：在启动时，代理只加载每个可用 skill 的名称和描述，足以知道何时可能相关
...
每个 skill 以包含 YAML 前置元数据和 Markdown 指令的 `SKILL.md` 文件开始：
...
前置元数据后的 Markdown 正文包含 skill 指令。没有格式限制。编写任何有助于代理有效执行任务的内容。
...
支持的语言取决于代理实现。常见选项包括Python、Bash和JavaScript。
...
## Agent Skills 能够实现的功能

- **领域专业知识**：将专门知识打包成可重复使用的指令，从法律审查流程到数据分析管道
- **新能力**：为代理提供新能力（例如创建演示文稿、构建MCP服务器、分析数据集）
- **可重复的工作流程**：将多步骤任务转换为一致且可审计的工作流程
- **互操作性**：在不同的兼容skills的代理产品中重用相同的skill
...
**基于文件系统的代理**在计算机环境（bash/unix）中运行，代表最有能力的选项。当模型发出shell命令如`cat /path/to/my-skill/SKILL.md`时激活Skills。通过shell命令访问捆绑资源。

**基于工具的代理**在没有专用计算机环境的情况下运行。相反，它们实现允许模型触发skills并访问捆绑资产的工具。具体的工具实现由开发者决定。
...
Skills是包含`SKILL.md`文件的文件夹。您的代理应该扫描配置目录以查找有效的skills。

### 元数据加载

在启动时，只解析每个`SKILL.md`文件的前置元数据。这保持了初始上下文使用量低。

对于Claude模型，推荐格式使用XML：
...
Skills应该为有效使用上下文而结构化：

1. **元数据**（约100个tokens）：在启动时为所有skills加载`name`和`description`字段
2. **指令**（建议<5000个tokens）：激活skill时加载完整的`SKILL.md`正文
3. **资源**（按需）：只有在需要时才加载文件（例如`scripts/`、`references/`或`assets/`中的文件）

保持主要的`SKILL.md`在500行以下。将详细的参考资料移到单独的文件中。
...
Agent Skills格式最初由Anthropic开发，作为开放标准发布，并已被越来越多的代理产品采用。该标准对来自更广泛生态系统的贡献开放。

## 参考工具

[skills-ref](https://github.com/agentskills/agentskills/tree/main/skills-ref)参考库提供Python实用程序和CLI来处理skills：
...
Agent Skills格式是开源的，项目托管在GitHub上：https://github.com/agentskills/agentskills

该标准欢迎来自更广泛生态系统的贡献和改进。